...
select avg(year_of_issue) as AVARAGE_year from cars;
...
select brand ,sum(price) from cars where brand='Mercedes-Benz' group by brand;


--#13 Эн кымбат жана эн арзан машинаны чыгарыныз
select (
           select brand from cars where price=(select max(price) from cars))as Expensive,
       (select max(price) from cars),
       (select brand  from cars where price=(select min (price) from cars)) as Cheap,
